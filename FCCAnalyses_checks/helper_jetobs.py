...
def plot_nconst(df_1, df_2, l1="df1", l2="df2", save=False, figsize=(12, 8)):
    # Mu, El, nnhad, nchad, ngamma
    fig, axs = plt.subplots(2, 3, figsize=figsize)
    labels = [l1, l2]
    for i, df in enumerate([df_1, df_2]):
        nmu = np.array([np.sum(arr) for arr in df["pfcand_isMu"]])
        nel = np.array([np.sum(arr) for arr in df["pfcand_isEl"]])
        nnnhad = np.array([np.sum(arr) for arr in df["pfcand_isNeutralHad"]])
        nnchad = np.array([np.sum(arr) for arr in df["pfcand_isChargedHad"]])
        nngamma = np.array([np.sum(arr) for arr in df["pfcand_isGamma"]])
        nconst = np.array([len(arr) for arr in df["pfcand_e"]])

        axs[0, 0].hist(nmu, bins=4, histtype='step', density=True, linewidth=2.0, label=labels[i], range=(0, 4))
        axs[0, 1].hist(nel, bins=7, histtype='step', density=True, linewidth=2.0, label=labels[i], range=(0, 7))
        axs[0, 2].hist(nnnhad, bins=20, histtype='step', density=True, linewidth=2.0, label=labels[i], range=(0, 20))
        axs[1, 0].hist(nnchad, bins=35, histtype='step', density=True, linewidth=2.0, label=labels[i], range=(0, 35))
        axs[1, 1].hist(nngamma, bins=35, histtype='step', density=True, linewidth=2.0, label=labels[i], range=(0, 35))
        axs[1, 2].hist(nconst, bins=75, histtype='step', density=True, linewidth=2.0, label=labels[i], range=(0, 75))

    axs[0, 0].set_xlabel("# Muons")
    axs[0, 1].set_xlabel("# Electrons")
    axs[0, 2].set_xlabel("# Neutral hadrons")
    axs[1, 0].set_xlabel("# Charged hadrons")
    axs[1, 1].set_xlabel("# Photons")
    axs[1, 2].set_xlabel("# Constituents")
    ...
    plt.tight_layout()
    if save:
        plt.savefig("plots/nconst_comparison.pdf")
    plt.show()
